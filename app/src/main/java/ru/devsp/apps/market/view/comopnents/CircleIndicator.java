...
/**
 * Индикатор страниц для ViewPager
 * Created by gen on 15.01.2018.
 */
...
public class CircleIndicator extends View implements ViewPager.OnPageChangeListener {
    ...
    private int mNumOfIndicators = 1;
    ...
    private void setDefaults(Context context) {
        ...
        mSelectedDrawable = ContextCompat.getDrawable(context, R.drawable.ic_page_indicator_selected);
        mUnSelectedDrawable = ContextCompat.getDrawable(context, R.drawable.ic_page_indicator_unselected);
    }

    private void getDataFromAttributes(Context context, @Nullable AttributeSet attrs) {
        if (attrs != null) {
            final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CircleIndicator, 0, 0);

            // Get indicators if exist
            Drawable selectedDrawable = a.getDrawable(R.styleable.CircleIndicator_selectedPageDrawable);
            if (selectedDrawable != null) {
                mSelectedDrawable = selectedDrawable;
            }
            Drawable unSelectedDrawable = a.getDrawable(R.styleable.CircleIndicator_pageDrawable);
            if (unSelectedDrawable != null) {
                mUnSelectedDrawable = unSelectedDrawable;
            }

            // Get indicator size
            mIndicatorSize = (int) a.getDimension(R.styleable.CircleIndicator_pageSize, mIndicatorSize);
            mIndicatorSelectedSize = (int) a.getDimension(R.styleable.CircleIndicator_selectedPageSize, mIndicatorSize);

            // Get padding between indicators
            mPaddingBetweenIndicators = (int) a.getDimension(R.styleable.CircleIndicator_margin, mPaddingBetweenIndicators);
            mSmoothTransitionEnabled = a.getBoolean(R.styleable.CircleIndicator_snap, true);
